...
def get_depth(node, graph, depth=0, pass_nodes=None):
    ...
    for sub in graph.get(node, []):
        if sub not in pass_nodes:
            depths.append(get_depth(sub, graph, depth+1, pass_nodes))
        # depths.append(get_depth(sub, graph, depth+1, pass_nodes))
    return max(depths) if depths else depth+1


def get_breadths(node, graph, depth=0, breadths=None, pass_nodes=None):
    if breadths is None:
    ...
    for sub in graph.get(node, []):
        if sub not in pass_nodes:
            ...
            get_breadths(sub, graph, depth+1, breadths, pass_nodes)
        # if sub not in breadths[depth+1]:
        #     breadths[depth+1].append(sub)
...
class Transition(Viewer):
    ...
    * stage: The contents of the current pipeline stage.
    ...
    def __init__(self, stages=[], graph={}, root=None, **params):
        try:
        ...
        self.define_graph(graph, root=root)
    ...
    def define_graph(self, graph, root=None, force=False):
        """
        ...
        if not is_traversable(self._root, graph, stages):
