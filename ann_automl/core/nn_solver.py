...
def train_classification_model(classes,
    ...
    res = tune(task, tuned_params='all', method='grid', hparams=hparams, stop_flag=stop_flag, verbosity=verbosity)
...
def create_classification_model(classes,
    ...
    """
    if verbosity > 0:
        printlog(f'Запускаем создание модели, это может занять длительное время.\n'
    ...
    if val < target_accuracy:
        ...
        printlog(f'Модель готова. Полученная точность на тестовой выборке: {val}')
    ...
    if script_type is not None:
        if script_type not in ('tf', 'torch'):
            printlog(f'Указан неправильный тип скрипта: {script_type}, должен быть tf или torch', file=sys.stderr)
            printlog('Используем тип tf', file=sys.stderr)
            script_type = 'tf'
        print('Копируем скрипт для классификации')
        copy_classify_script(save_dir)
        info['backend'] = script_type

    if output_dir.endswith('.zip'):
        printlog(f'Создаём архив {output_dir}')
