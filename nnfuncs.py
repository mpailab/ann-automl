...
def fit_segmentation(nn_task, model, generators, hparams, stop_flag, timeout, cur_subdir, check_metric,  use_tensorboard,
    ...
    model = get_local_model(img_size=oxf_param, num_classes=len(class_ids))
...
def fit_detection(nn_task, model, generators, hparams, stop_flag, timeout, cur_subdir, check_metric,  use_tensorboard,
    ...
    model = keras_cv.models.RetinaNet.from_preset(
        hparams.get('model_architecture', "resnet50_imagenet"),
...
def create_and_train_model(nn_task, hparams, objects, data, cur_subdir, history=None, stop_flag=None,
    ...
    if nn_task.type == 'classification':
        ...
        ####
        #TODO: transform data to tf.data.Dataset (look at Dataset format.txt)
        #test_ratio = hparams.get('test_frac', 0.15)
        #val_ratio = hparams.get('val_frac', 0.15)
        #parts = [1-test_ratio-val_ratio,val_ratio, test_ratio]

        pascal_shape = 640
        train_ds = load_pascal_voc(split="train+test[:40%]", dataset="voc/2007", bounding_box_format=hparams.get('bounding_box_format', "xywh"))
        eval_ds = load_pascal_voc(split="test[40%:70%]", dataset="voc/2007", bounding_box_format=hparams.get('bounding_box_format', "xywh"))
        test_ds = load_pascal_voc(split="test[70%:]", dataset="voc/2007", bounding_box_format=hparams.get('bounding_box_format', "xywh"))
        
        train_ds = train_ds.shuffle(BATCH_SIZE * 4)
    
        train_ds = train_ds.ragged_batch(BATCH_SIZE, drop_remainder=hparams.get('drop_remainder', True))
        eval_ds = eval_ds.ragged_batch(BATCH_SIZE, drop_remainder=hparams.get('drop_remainder', True))
        test_ds = test_ds.ragged_batch(BATCH_SIZE, drop_remainder=hparams.get('drop_remainder', True))
        ...
        augmenter = keras.Sequential(
            layers=[
                ...
                keras_cv.layers.JitteredResize(
                    target_size=(pascal_shape, pascal_shape), scale_factor=(hparams.get('resize_min', 0.75), hparams.get('resize_max', 1.3)),
                    bounding_box_format=hparams.get('bounding_box_format', "xywh")
                ),
            ]
        )
        ...
        inference_resizing = keras_cv.layers.Resizing(pascal_shape, pascal_shape, bounding_box_format=hparams.get('bounding_box_format', "xywh"),
        ...
        generators = {'train' : train_ds, 'eval' : eval_ds, 'test' : test_ds}
        ...
        orig_train_ds, orig_val_ds, orig_test_ds  = tfds.load(name="oxford_iiit_pet", split=["train+test[:40%]", "test[40%:70%]", "test[70%:]"],
                                                           )
        rescale_images_and_correct_masks = lambda inputs: {"images": tf.cast(inputs["image"], dtype=tf.float32) / 255.0,
        ...
        resize_fn = keras_cv.layers.Resizing(oxf_param[0], oxf_param[1])
        augment_fn = keras.Sequential(
            layers=[
                resize_fn,
                keras_cv.layers.RandomFlip(mode=hparams.get('augmen_params_flip', "horizontal_and_vertical")),
                keras_cv.layers.RandomRotation( factor=(hparams.get('ROTATION_FACTOR_min', -0.2),hparams.get('ROTATION_FACTOR_max', 0.2)),
                                               segmentation_classes=len(class_ids)),        
            ])

        augmented_train_ds = (train_ds.shuffle(BATCH_SIZE * 2).map(augment_fn, num_parallel_calls=AUTOTUNE).batch(BATCH_SIZE).map(unpackage_inputs).prefetch(buffer_size=AUTOTUNE))
        resized_val_ds = ( val_ds.map(resize_fn, num_parallel_calls=AUTOTUNE).batch(BATCH_SIZE).map(unpackage_inputs).prefetch(buffer_size=AUTOTUNE))
        resized_test_ds = ( test_ds.map(resize_fn, num_parallel_calls=AUTOTUNE).batch(BATCH_SIZE).map(unpackage_inputs).prefetch(buffer_size=AUTOTUNE))

        generators = {'train' : augmented_train_ds, 'eval' : resized_val_ds, 'test' : resized_test_ds}
